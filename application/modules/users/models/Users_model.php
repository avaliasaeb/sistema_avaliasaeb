...
class Users_model extends MY_Model{
  ...
  /**
   * Obtém lista com todos os usuários ativos
   *
   * @param   none
   * @return  array/false
   */
  public function getActiveUsers(){
    ...
    $this->db->join( 'perfil p', 'u.id_perfil = p.id' );
    $this->db->join( 'parametro_detalhe a', 'u.id_pais = a.id', 'left' ); 
    $this->db->join( 'parametro_detalhe d', 'u.id_tipo_documento = d.id', 'left' ); 
    $this->db->join( 'parametro_detalhe n', 'u.id_necessidade = n.id', 'left' ); 
    $this->db->order_by( 'u.id' );
    $q = $this->db->get();
    return $q->num_rows() > 0 ? $q->result_array() : false;
   ...
   * @param   date - $arrData['dt_login'] (Data do Login)
   * @return  array/false
   */
  public function getAutenticatedUser( $arrData ){
    $this->db->select( 'u.id' );
    $this->db->select( 'u.nome' );
    $this->db->select( 'SUBSTRING_INDEX(u.nome, " ", 1) primeiro_nome' );
    $this->db->select( 'u.email' );
    $this->db->select( 'u.login' );
    $this->db->select( 'u.idioma' );
    $this->db->select( 'u.id_perfil' );
    ...
    if( array_key_exists('id', $arrData) ){
        $this->db->where( 'u.id', $arrData['id'] );
    }else{
      $this->db->where( 'u.login', $arrData['login'] );
      $this->db->where( 'u.senha', $arrData['senha'] );
    }
    ...
    //se houver usuário com as credenciais informadas
    if( $q->num_rows() === 1 ){
      $arrUser = $q->result_array()[0];

      //Obtem a lista atualizada de permissões do usuário logado
      if( $arrUser['permissoes'] == '*'){
        $arrUser['roles'] = '*';
      }else{

        $this->db->select('permissao');
        $this->db->from('permissao');
        $this->db->where_in( 'id', explode( ',', $arrUser['permissoes'] ) );
        $p = $this->db->get();
        $arrRoles = $p->result_array();

        $strRole = '';
        foreach( $arrRoles as $role ){
          $strRole .= $role['permissao'] . ';';
        }

        $arrUser['roles'] = implode( ';', array_filter( array_unique( array_map('trim', explode( ';', $strRole ) ) ) ) );
      }
      return $arrUser;
    }
    return false;
   ...
   * @return  bool
   */
  public function setUser( $arrData ){
