...
class User extends Private_Controller {
	...
	public function index( $intID = null ){
		...
		$this->_setPageTitle( $this->lang->line('page_title_user') );
    $this->load->model('users_model');
		$this->data['item'] = $this->users_model->getUser( $intID );
	...
	public function insert(){
		$this->_setPageTitle( $this->lang->line('page_title_user_add') );
    $this->load->model('users_model');
    $this->load->model('roles/roles_model');
    $this->load->model('params/params_model');
		$this->data['list'] = $this->roles_model->getActiveRoles();
	...
	public function edit( $intID = null ){
		...
		$this->_setPageTitle( $this->lang->line('page_title_user_edt') );
    $this->load->model('users_model');
    $this->load->model('roles/roles_model');
    $this->load->model('params/params_model');
		$this->data['item'] = $this->users_model->getUser( $intID );
    ...
    $arrData['id_perfil'] = $this->_input('input_perfil');
    ...
    $this->load->model('users_model');
    if( true === $this->users_model->loginAlreadyExists( $arrData['login'] ) ){
	    $output = array(
	       'status'	=> false
    ...
    $bolInseriu = $this->users_model->setUser( $arrData );
    ...
    $this->load->model('users_model');
    $bolAtualizou = $this->users_model->updateUser( $arrData );
    ...
    $bolAtualizou = $this->users_model->updateUserLogin( $arrData );
    ...
    $this->load->model('users_model');
    if( true === $this->users_model->udpateStatus( $arrData ) ){
	    $output = array(
	       'status'	=> true
	      ,'msg' 		=> $this->lang->line('ajax_6')
	      ,'href'		=> site_url( $this->router->fetch_module() )
