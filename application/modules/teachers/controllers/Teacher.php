...
class Teacher extends Private_Controller {
	...
	public function index( $intID = null ){
		...
		$this->_setPageTitle( $this->lang->line('page_title_teacher') );
    $this->load->model('teachers_model');
		$this->data['item'] = $this->teachers_model->getTeacher( $intID );
		$this->load->view('main/header_view', $this->data);
		$this->load->view('teacher_view', $this->data);
	...
	public function insert(){
		$this->_setPageTitle( $this->lang->line('page_title_teachers_add') );
    $this->load->model('teachers_model');
	...
	public function edit( $intID = null ){
		...
		$this->_setPageTitle( $this->lang->line('page_title_teachers_edt') );
    $this->load->model('teachers_model');
    $this->load->model('roles/roles_model');
    $this->load->model('params/params_model');
		$this->data['item'] = $this->teachers_model->getTeacher( $intID );
    ...
    $arrData['id_perfil'] = $this->config->item('perfil_professor');
    ...
    $this->load->model('teachers_model');
    if( true === $this->teachers_model->loginAlreadyExists( $arrData['login'] ) ){
	    $output = array(
	       'status'	=> false
    ...
    $bolInseriu = $this->teachers_model->setTeacher( $arrData );
    ...
    $this->load->model('teachers_model');
    $bolAtualizou = $this->teachers_model->updateUser( $arrData );
    ...
    $this->load->model('teachers_model');
    if( true === $this->teachers_model->anotherLoginAlreadyExists( $arrData ) ){
	    $output = array(
	       'status'	=> false
	      ,'msg' 		=> $this->lang->line('ajax_8')
	    );
    ...
    $bolAtualizou = $this->teachers_model->updateUserLogin( $arrData );
    ...
    $this->load->model('teachers_model');
    if( true === $this->teachers_model->udpateStatus( $arrData ) ){
	    $output = array(
	       'status'	=> true
	      ,'msg' 		=> $this->lang->line('ajax_6')
	      ,'href'		=> site_url( $this->router->fetch_module() )
