			  		...
			  		<form>
			  			<div class="row">
							...
							<div class="d-flex justify-content-between">
								<a class="btn btn-lg btn-outline-dark" href="<?= site_url('teachers') ?>"><i class="fa-solid fa-chevron-left"></i> <?= $this->lang->line('form_voltar') ?></a>
								<?php if( $controller->_hasUserPermission('teacher/teacher/edit') ){ ?>
								<?php //o usuário não pode editar a si mesmo
						      if( $item['id'] !== $user['id'] ){ ?>
								<a class="btn btn-lg btn-dark" href="<?= site_url('teachers/teacher/edit/') ?><?= $item['id'] ?>"><i class="far fa-edit"></i> <?= $this->lang->line('form_editar') ?></a>
								<?php } ?>
								<?php } ?>
							</div>
			  		</form>
			  	...
			  	<div class="card-body">
			  		...
			  		<form>
			  			<div class="form-floating mb-3">
							...
							<div class="d-flex justify-content-between">
								<a class="btn btn-lg btn-dark" href="<?= site_url('teachers') ?>"><i class="fa-solid fa-chevron-left"></i> <?= $this->lang->line('form_voltar') ?></a>
								<?php if( $controller->_hasUserPermission('teachers/teacher/edit') ){ ?>
								<?php //o usuário não pode editar a si mesmo
						      if( $item['id'] !== $user['id'] ){ ?>
								<a class="btn btn-lg btn-outline-dark" href="<?= site_url('teachers/teacher/edit/') ?><?= $item['id'] ?>"><i class="far fa-edit"></i> <?= $this->lang->line('form_editar') ?></a>
								<?php } ?>
								<?php } ?>
							</div>
			  		</form>
